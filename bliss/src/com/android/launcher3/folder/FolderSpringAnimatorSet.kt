 ...
 */
/*
 * File:    bliss/src/com/android/launcher3/folder/FolderSpringAnimatorSet.kt
 ...
 *   ├── FolderAnimationData.kt                  — folder open/close geometry + scaling
 ...
 *   ├── FolderSpringAnimatorSet.kt              — owns the AnimatorSet built per open/close  ← THIS FILE
 *   └── IconAnimationData.kt                    — per-preview-icon animation tuple
 *
 * Purpose:
 *   Orchestrates the SpringAnimationBuilder calls that translate a
 *   pre-computed FolderAnimationData / ClipRevealData / IconAnimationData
 *   tuple into a runnable AnimatorSet for the spring-physics open/close
 *   path. Migration04 §06 audit pass: this file performs no measure-time
 *   divisions of its own — every float it feeds into SpringAnimationBuilder
 *   is taken verbatim from FolderAnimationData (already FloatGuard-validated)
 *   or is a constant. No FloatGuard call sites added here; header refreshed
 *   to Form A so future maintainers know it was reviewed.
 *
 * Consumed by:
 *   - com.android.launcher3.folder.FolderAnimationSpringBuilderManager
 *
 * Plan reference: Plans/Migration04/06-animation-safety.md §5.3
 */
...
class FolderSpringAnimatorSet(val animatorSet: AnimatorSet) {
    ...
    companion object Factory {
        ...
        private fun setupFolder(folder: Folder, folderAnimationData: FolderAnimationData) {
            folder.mFolderIcon.previewItemManager.recomputePreviewDrawingParams()
            folder.apply {
                pivotX = 0f
                pivotY = 0f
            }
