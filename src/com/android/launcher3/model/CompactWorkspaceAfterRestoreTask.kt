...
class CompactWorkspaceAfterRestoreTask : BaseModelUpdateTask() {
    ...
    override fun execute(app: LauncherAppState, dataModel: BgDataModel, apps: AllAppsList) {
        ...
        movableItems.forEach { item ->
            ...
            if (!placed) {
                val newScreenId = nextScreenId(screenIds)
        ...
        updated.forEach { modelWriter.updateItemInDatabase(it) }
        model.forceReload()
    }
    ...
    private fun nextScreenId(existingScreenIds: List<Int>): Int {
        var maxScreenId = FIRST_SCREEN_ID
